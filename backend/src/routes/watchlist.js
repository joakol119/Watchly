  ...
  try {
    await Promise.all(
      items.map(({ id, position }) =>
        db.query('UPDATE watchlist SET position = $1 WHERE id = $2 AND user_id = $3', [position, id, req.user.id])
      )
    );
...
router.patch('/:id', auth, async (req, res) => {
  ...
  try {
    ...
    if (user_rating !== undefined) {
      const rating = user_rating === null ? null : Math.min(10, Math.max(1, parseInt(user_rating)));
      result = await db.query(
        'UPDATE watchlist SET user_rating = $1 WHERE id = $2 AND user_id = $3 RETURNING *',
