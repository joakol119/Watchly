              ...
              <div style={{ position: 'relative', zIndex: 1, maxWidth: 1200, margin: '0 auto', padding: '0 32px', height: '100%', display: 'flex', alignItems: 'center' }}>
                <div style={{ maxWidth: 520, animation: 'fadeUp 0.6s ease forwards' }}>
                  ...
                  <div style={{ display: 'flex', gap: 12 }}>
                    <button className="featured-btn-primary" onClick={() => router.push(`/${featured.media_type}/${featured.id}`)} style={{ padding: '12px 28px', borderRadius: 12, border: 'none', background: 'linear-gradient(135deg, #e50914, #b81c1c)', color: '#fff', fontWeight: 700, cursor: 'pointer', fontSize: 14, fontFamily: 'inherit', transition: 'all 0.2s', boxShadow: '0 4px 16px rgba(229,9,20,0.3)' }}>
                      Ver detalles →
                    </button>
                    {!watchlistMap[`${featured.media_type}-${featured.id}`] ? (
                      <button className="featured-btn-secondary" onClick={() => handleAdd(featured, featured.media_type)} style={{ padding: '12px 24px', borderRadius: 12, border: '1px solid rgba(255,255,255,0.15)', background: 'rgba(255,255,255,0.08)', color: '#f1f5f9', fontWeight: 600, cursor: 'pointer', fontSize: 14, fontFamily: 'inherit', transition: 'all 0.2s' }}>
                        + Mi lista
                      </button>
                    ) : (
                      <button className="featured-btn-secondary" style={{ padding: '12px 24px', borderRadius: 12, border: '1px solid rgba(99,102,241,0.3)', background: 'rgba(99,102,241,0.1)', color: '#a5b4fc', fontWeight: 600, cursor: 'default', fontSize: 14, fontFamily: 'inherit' }}>
                        ✓ En mi lista
                      </button>
                    )}
                  </div>
