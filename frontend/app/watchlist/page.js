...
export default function WatchlistPage() {
  ...
  const handleRating = async (e, item, rating) => {
    ...
    } catch (err) { addToast('No se pudo guardar la calificación', 'error'); }
  };
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '88px 32px 80px' }}>
        ...
        <div style={{ marginBottom: 40 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
            <h1 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 52, fontWeight: 400, letterSpacing: 1, color: '#fff', lineHeight: 1 }}>Mi lista</h1>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 36, flexWrap: 'wrap' }}>
          {filters.map(f => (
            <button key={f.value} className="filter-btn" onClick={() => setFilter(f.value)}
              style={{ padding: '9px 20px', borderRadius: 10, border: '1px solid', borderColor: filter === f.value ? 'rgba(229,9,20,0.4)' : 'rgba(255,255,255,0.08)', background: filter === f.value ? 'rgba(229,9,20,0.15)' : 'rgba(255,255,255,0.04)', color: filter === f.value ? '#f87171' : '#64748b', cursor: 'pointer', fontSize: 13, fontWeight: 600, fontFamily: 'inherit', transition: 'all 0.2s', display: 'flex', alignItems: 'center', gap: 8 }}>
              {f.label}
              <span style={{ background: 'rgba(255,255,255,0.08)', borderRadius: 999, padding: '1px 8px', fontSize: 11, color: filter === f.value ? '#f87171' : '#475569' }}>{counts[f.value]}</span>
            </button>
          ))}
        </div>

        {filtered.length > 1 && (
          <p style={{ fontSize: 12, color: '#334155', marginBottom: 20 }}>⠿ Arrastrá las cards para reordenar tu lista</p>
        )}

        {loading ? (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 20 }}>
            {[...Array(8)].map((_, i) => <div key={i} style={{ borderRadius: 14, paddingBottom: '150%', background: '#141420', animation: 'pulse 1.5s ease-in-out infinite', animationDelay: `${i * 0.07}s` }} />)}
          </div>
        ) : filtered.length === 0 ? (
            ...
            {filtered.map((item, i) => {
              ...
              return (
                <div key={item.id} className="watchlist-card"
                  ...
                  style={{ borderRadius: 14, overflow: 'hidden', background: '#141420', border: `1px solid ${isDragOver ? 'rgba(229,9,20,0.5)' : 'rgba(255,255,255,0.06)'}`, cursor: 'grab', transition: 'all 0.2s', position: 'relative', animation: `fadeUp 0.4s ease forwards`, animationDelay: `${i * 0.04}s`, opacity: isDragging ? 0.4 : 0, transform: isDragOver ? 'scale(1.03)' : 'scale(1)' }}>
                  ...
                  <div style={{ position: 'relative', paddingBottom: '150%', background: '#1e1e2e' }}>
                    ...
                    <div className="card-overlay" style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(8,8,16,0.98) 50%, rgba(8,8,16,0.5) 100%)', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: 12 }} onClick={e => e.stopPropagation()}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginBottom: 8 }}>
                        {STATUS_OPTIONS.map(s => (
                          <button key={s.value} className="status-option" onClick={e => handleStatus(e, item, s.value)}
                            style={{ padding: '6px 10px', borderRadius: 8, border: '1px solid', borderColor: item.status === s.value ? s.border : 'rgba(255,255,255,0.08)', background: item.status === s.value ? s.bg : 'rgba(255,255,255,0.04)', color: item.status === s.value ? s.color : '#64748b', cursor: 'pointer', fontSize: 11, fontWeight: 600, fontFamily: 'inherit', textAlign: 'left', transition: 'all 0.15s' }}>
                            {s.label}
                          </button>
                        ))}
                      </div>

                      {/* Star rating — only if watched */}
                      {item.status === 'watched' && (
                        <div style={{ marginBottom: 8 }}>
                          <p style={{ fontSize: 10, color: '#475569', marginBottom: 4, textTransform: 'uppercase', letterSpacing: 0.5 }}>Tu calificación</p>
                      ...
                      <button onClick={e => handleRemove(e, item.id)}
                        style={{ padding: '6px', borderRadius: 8, border: '1px solid rgba(239,68,68,0.3)', background: 'rgba(239,68,68,0.1)', color: '#f87171', cursor: 'pointer', fontSize: 11, fontWeight: 600, fontFamily: 'inherit' }}>
                        🗑 Eliminar
                      </button>
                    </div>
                  </div>
