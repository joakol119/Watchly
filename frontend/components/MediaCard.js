...
export default function MediaCard({ item, onAdd, watchlistMap }) {
  ...
  return (
    <div onClick={() => router.push(`/${type}/${item.id}`)} style={{ cursor: 'pointer', borderRadius: 12, overflow: 'hidden', background: '#141420', border: '1px solid rgba(255,255,255,0.06)', transition: 'all 0.2s', position: 'relative' }}
      ...
      <div style={{ padding: '12px' }}>
        ...
        {onAdd && (
          <button onClick={e => { e.stopPropagation(); onAdd(item, type); }} style={{ width: '100%', padding: '7px', borderRadius: 8, border: 'none', background: inList ? 'rgba(99,102,241,0.2)' : 'rgba(229,9,20,0.15)', color: inList ? '#a5b4fc' : '#f87171', fontWeight: 600, cursor: 'pointer', fontSize: 12, fontFamily: 'inherit' }}>
            {inList ? '✓ En mi lista' : '+ Agregar'}
          </button>
        )}
      </div>
    </div>
